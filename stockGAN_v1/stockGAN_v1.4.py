...
class GAN():
    def __init__(self,batch_size):
        ...
        if os.path.isfile('trainX.sav') is True:
            ...
            print(self.GAN_trainY.shape)
            for idx in reversed(range(self.gen_output)):
                if idx % 5 == 4:
                    self.GAN_trainSTOCK = np.delete(self.GAN_trainSTOCK, idx, axis=1)
            print(self.GAN_trainSTOCK.shape)
            for idx in reversed(range(self.gen_output)):
                if idx % 5 == 4:
                    self.GAN_testX = np.delete(self.GAN_testX, idx, axis=2)
            print(self.GAN_testX.shape)
            for idx in reversed(range(self.gen_output)):
                if idx % 5 == 4:
                    self.GAN_testY = np.delete(self.GAN_testY, idx, axis=1)
            print(self.GAN_testY.shape)
            for idx in reversed(range(self.gen_output)):
                if idx % 5 == 4:
                    self.GAN_testSTOCK = np.delete(self.GAN_testSTOCK, idx, axis=1)
            print(self.GAN_testSTOCK.shape)
