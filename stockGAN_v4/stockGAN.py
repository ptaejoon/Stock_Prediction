...
#        self.GAN_trainSTOCK = self.scaler.transform(self.GAN_trainSTOCK)
        ...
        for times in range(time):
            ...
            print("실제 Percent 대조값 Training : "+str(np.sum(np.abs(self.revserse_scaling(predict_result)-self.revserse_scaling(self.GAN_trainY)))))
            print("실제 Percent 대조값 Validation : "+str(np.sum(np.abs(self.revserse_scaling(predict_val)-self.revserse_scaling(self.GAN_testY)))))
