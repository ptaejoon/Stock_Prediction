...
class GAN():
    def __init__(self,batch_size):
        ...
        trainX_transformed = self.scaler.inverse_transform(self.GAN_trainX[:,:,:self.gen_output].reshape(-1,self.gen_output)).copy()
        testX_transformed = self.scaler.inverse_transform(self.GAN_testX[:,:,:self.gen_output].reshape(-1,self.gen_output)).copy()
        ...
        for idx in reversed(range(self.gen_output)):
            if idx % 5 == 4:
                testX_transformed = np.delete(testX_transformed, idx, axis=1)
        for idx in reversed(range(self.gen_output)):
            if idx % 5 == 4:
                trainX_transformed = np.delete(trainX_transformed, idx, axis=1)
        for idx in reversed(range(self.gen_output)):
            if idx % 5 == 4:
                self.GAN_trainX = np.delete(self.GAN_trainX, idx, axis=2)
        for idx in reversed(range(self.gen_output)):
            if idx % 5 == 4:
                self.GAN_trainY = np.delete(self.GAN_trainY, idx, axis=1)
        for idx in reversed(range(self.gen_output)):
            if idx % 5 == 4:
                self.GAN_testX = np.delete(self.GAN_testX, idx, axis=2)
        for idx in reversed(range(self.gen_output)):
            if idx % 5 == 4:
                self.GAN_testY = np.delete(self.GAN_testY, idx, axis=1)
        ...
        self.gen_output = int(self.gen_output*4/5)
        self.stock_size = self.gen_output
        self.gen_feature = self.stock_size+self.pv_size
        ...
        past_stock = keras.Input(shape=((self.gen_timestep),self.stock_size), name='past_stock')
    ...
    def build_discriminator(self):
        model = keras.Sequential()
        model.add(keras.layers.Conv1D(32, kernel_size=4,strides=2,input_shape=(self.gen_timestep+1,self.stock_size),data_format='channels_first'))
        ...
        model.summary()
        estimated_sequence = keras.Input(shape=(self.gen_timestep+1,self.stock_size))
    ...
    def numpy_one_day_data(self,newsDB,stockDB,days,pv):
        ...
        stock_sql = """select CLOSE_PRICE, OPEN_PRICE, MAX_PRICE, MIN_PRICE
            from stock where TRADE_TIME >= %s and TRADE_TIME < %s and
            corp_name in (
            select distinct corp_name from stock 
...
#        self.GAN_trainSTOCK = self.scaler.transform(self.GAN_trainSTOCK)
        ...
        for times in range(time):
            ...
            print(np.sum(np.abs((self.scaler.inverse_transform(predict_val) - self.scaler.inverse_transform(self.GAN_testY)))))
    ...
    def predict(self, days):  # y hat
        ...
        pv_model = gensim.models.Doc2Vec.load('vocab_20.model')
        target_day = days
        days = days - datetime.timedelta(days=self.gen_timestep*2)
        np_pv = np.empty(self.pv_size)
        np_stock = np.empty(self.stock_size)

        for i in range(self.gen_timestep*2):
            if days.date() >= target_day.date():
                break
            oneday, stock, days = self.numpy_one_day_data(articleDBconnect, stockDBconnect, days, pv_model)
            np_pv = np.vstack([np_pv, oneday])
            np_stock = np.vstack([np_stock, stock])
