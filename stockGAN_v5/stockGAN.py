...
class GAN():
    def __init__(self,batch_size):
        ...
        self.GAN_trainY = pickle.load(open('v5_trainY.sav', 'rb'))
        GAN_trainX_STOCK = pickle.load(open('v5_trainX_STOCK.sav','rb'))
        GAN_trainX_PV = pickle.load(open('trainX_PV.sav','rb'))
        self.GAN_testY = pickle.load(open('v5_testY.sav','rb'))
        GAN_testX_STOCK = pickle.load(open('v5_testX_STOCK.sav','rb'))
        GAN_testX_PV = pickle.load(open('testX_PV.sav','rb'))
        self.GAN_valY=pickle.load(open('v5_valY.sav','rb'))
        GAN_valX_STOCK = pickle.load(open("v5_valX_STOCK.sav",'rb'))
        GAN_val_PV = pickle.load(open('val_PV.sav','rb'))
        GAN_val_PV = np.delete(GAN_val_PV,(0),axis=0)
        ...
        self.GAN_valX = np.concatenate((GAN_valX_STOCK,GAN_val_PV),axis=2)
        self.GAN_trainSTOCK = pickle.load(open('v5_trainSTOCK.sav','rb'))
        self.GAN_testSTOCK =  pickle.load(open('v5_testSTOCK.sav','rb'))
        self.GAN_valSTOCK = pickle.load(open('v5_valSTOCK.sav','rb'))
    ...
    def revserse_scaling(self,result):
        result = (result + 1) / 2 * 0.6 + 0.7
        return result
...
#        self.GAN_trainSTOCK = self.scaler.transform(self.GAN_trainSTOCK)
        ...
        for times in range(time):
            ...
            predict_test= self.generator.predict(self.GAN_testX).reshape(-1,self.gen_output)
            predict_val = self.generator.predict(self.GAN_valX).reshape(-1,self.gen_output)
            print(np.sum(np.abs(predict_result - self.GAN_trainY)))
            print(np.sum(np.abs(predict_test - self.GAN_testY)))
            print(np.sum(np.abs(predict_val-self.GAN_valY)))
